...
class SpatialTransformation(nn.Module):
    """
    The application of deformation field to the image
    with the ability to compute gradient.
    """
    ...
    def meshgrid(self, height, width):
        ...
        y_t = torch.matmul(torch.unsqueeze(torch.linspace(0.0, height - 1.0, height), 1),
                           torch.ones([1, width]))
